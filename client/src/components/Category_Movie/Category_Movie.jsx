...
function Category_Movie() {
    ...
    const deleteSubmit = async (CategoryID) => {
        try {
            ...
            if (confirmation.isConfirmed) {
                const response = await axios.post(`http://localhost:5231/api/CategoryMovie/DeleteCategory/${CategoryID}`);
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        if (updatename == '') {
            ...
            try {
                const response = await fetch(`http://localhost:5231/api/CategoryMovie/updateCategory/${id}`, {
                    method: 'POST',
                    headers: {
    ...
    const handleEditClick = (CategoryID) => {
        const selectedCategory = categoryMovie.find(category => category.id == CategoryID)
        if (selectedCategory) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (name === '') {
            ...
            try {
                const response = await fetch('http://localhost:5231/api/CategoryMovie/Add', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (!response.ok) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Add Category Movie success',
                        showConfirmButton: false,
                        timer: 1500,
                    })
                    setName('')
    ...
    const filteredCategories = categoryMovie.filter(category =>

        category.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
    const indexOflastcategory = (currentPage + 1) * perPage;
    const indexOfFirtcategory = indexOflastcategory - perPage;
    const currentGender = filteredCategories.slice(indexOfFirtcategory, indexOflastcategory)
    return (
        <div>
                        ...
                        <ul className="sidebar-menu">
                            ...
                            <li className="treeview text-white">
                                ...
                                <a className='cursor-pointer' onClick={() => navigate('/Genre', { state: { username: username, ID: ID } })}>
                                   <i class="fa fa-list-alt" aria-hidden="true"></i> <span>Genre</span>
                                </a>
                        ...
                        <div className="row">
                            <div className="box box-primary" style={{ maxHeight: '542px' }}>
                                <div className="box-header">
                                    <h3 className="box-title">Category Movie</h3>
                                </div>
                            ...
                            <div className="box">
                                <div className="box-header">
                                    <h3 className="box-title">List Category Movie</h3>
                                </div>
                                ...
                                <div className="box-body">
                                    <table id="example1" className="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            {currentGender.map((categoryMovie, index) => (
                                                <tr key={categoryMovie.id}>
                                                    <td>{index + 1}</td>
                                                    <td>{categoryMovie.name}</td>
                                                    <td><button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={() => handleEditClick(categoryMovie.id)}>Edit</button></td>
                                                    <td><button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onClick={() => deleteSubmit(categoryMovie.id)}>Remove</button></td>
                                                </tr>
                                            ))}
                    ...
                    <div className="popup-container">
                        ...
                        <div className="popup-content" style={IsClosingPopup ? { ...popupContentStyle, ...closingAnimation } : popupContentStyle}>
                            ...
                            <div >
                                ...
                                <h3 className="box-title">Edit Category Movie</h3>
                            </div>
